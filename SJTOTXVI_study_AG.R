...
################################################################################
## The following are the codes of the Study of Brain Connectivity 
## (TOTXVI, St. Jude Children's Research Hospital)
...
lrld19 <- which(Xdat$Risk=="LR" & Xdat$IT.total <= 19) # number of low risk patients with low ITT dose
lrhd19 <- which(Xdat$Risk=="LR" & Xdat$IT.total > 19) # number of low risk patients with high ITT dose 
shrld26 <- which(Xdat$Risk=="SHR" & Xdat$IT.total <= 26) # number of standard-high risk patients with low ITT dose
shrhd26 <- which(Xdat$Risk=="SHR" & Xdat$IT.total > 26) # number of standard-high risk patients with high ITT dose

lp <- length(y_1[1,1,]) # total number of participants, control and patiens combined
...
plot(density(y_1[294, 242, cn]), ylim=c(-0.065, 1.1), xlim= c(-1, 5),
...
points(y_1[294, 242, shr], rep(-0.035, length(shr)), col="blue", pch= 16, cex=0.6)
...
plot(density(y_2[294, 242, cn]), ylim=c(-0.065, 1.1), xlim= c(-1, 5),
     main = "R Area 6A vs R Area 6mA", xlab ="Connectivity, Time = 2",
     cex.lab=1.4, cex.axis=1.3, cex.main=1.4)
abline(v=qntl[2], col= "red")
abline(v=qntl[1], col= "red")
points(y_2[294, 242, shr], rep(-0.035, length(shr)), col="blue", pch= 16, cex=0.6)
...
plot(density(y_diff1[cn]), ylim=c(-.065,1.1), xlim =c(-2.5, 2.5),
     main = "R Area 6A vs R Area 6mA", xlab ="Connectivity difference",
     cex.lab=1.4, cex.axis=1.3, cex.main=1.4)
abline(v=qntl[2], col= "red")
abline(v=qntl[1], col= "red")
points(y_diff1[shr], rep(-0.035, length(shr)), col="blue", pch= 16, cex=0.6)
...
plot(density(y_1[8,9, 1:59]), xlim=c(-1, 4.4), ylim=c(-0.065, 1.1),
     main = "L Accumbens vs L V Diencephalon", xlab ="Connectivity, time=1",
     cex.lab=1.4, cex.axis=1.3, cex.main=1.4)
abline(v=qntl[2], col= "red")
abline(v=qntl[1], col= "red")
points(y_1[8,9, shr], rep(-0.035, length(shr)), col="blue", pch= 16, cex=0.6)
...
# Next, the following table provides the count of edges that contain at least 50% ICC 
# and weights > 0.1 out of the 1093 edges chosen earlier.
...
rp <- 323  # the representative patient in the study
...
# get the mean connectivity at each pair of ROIs for the Standard-High Risk patients
# with low ITT dosages

y_1_meanSHRLD <- apply(y_1[,,shrld26], c(1,2), FUN = mean)
y_2_meanSHRLD <- apply(y_2[,,shrld26], c(1,2), FUN = mean)

l_mean <- c(c(y_1_meanSHRLD), c(y_2_meanSHRLD))
...
y_cor_control_meanSHRLD <- data.frame(Connectivity = Connectivity_mean,  
                                      Row= rep(c(1:379),2), 
                                      Column= rep(rep(c(1:379), each= 379), 2),
                                      Time= rep(c("RE-1", "W-120"), 
                                                each= length(c(y_1_meanSHRLD))) )


# get the mean connectivity at each pair of ROIs for the Standard-High Risk patients
# with high ITT dosages, ITT > 26

y_1_meanSHRHD <- apply(y_1[,,shrhd26], c(1,2), FUN = mean)
y_2_meanSHRHD <- apply(y_2[,,shrhd26], c(1,2), FUN = mean)

l_mean <- c(c(y_1_meanSHRHD), c(y_2_meanSHRHD))
...
y_cor_control_meanSHRHD <- data.frame(Connectivity = Connectivity_mean,  
                                      Row= rep(c(1:379),2), 
                                      Column= rep(rep(c(1:379), each= 379), 2),
                                      Time= rep(c("RE-1", "W-120"), 
                                                each= length(c(y_1_meanSHRHD))) )
...
dat_combine <- data.frame( rbind.data.frame(y_cor_control_rep, y_cor_control_meanC, 
                                            y_cor_control_meanSHRLD, y_cor_control_meanSHRHD),
                           pid= rep(c("Sample", "Mean: Control", "Mean: SHR(LD)",
                                      "Mean: SHR(HD)"),
                                    each= nrow(y_cor_control_rep)) )

dat_combine$pid <- factor(dat_combine$pid, levels=c("Sample", "Mean: Control", 
                                                    "Mean: SHR(LD)", "Mean: SHR(HD)"))
...
ggsave(file="Rplot_Connectivity_means.png", height =11, width =9,
       units ="in", dpi= 300)
...
###################################################
## For the patients in the control and LR groups ##
###################################################

cn1 <- c(cn, lr) # gathering the control group and LR patients
...
# histograms of age coefficients for comparison of Control vs SHR
...
# histograms of risk coefficients for comparison of Control vs SHR
...
#############################
## For Control vs SHR(LD) ##
#############################

cn2shrld <- c(cn, shrld26)

# subset the covariate data for control and low risk groups
xdat_cr2 <- Xdat[cn2shrld, ]
#xdat_cr2$Risk <- relevel(xdat_cr2$Risk, ref = "")
xdat_cr2$Age <- scale(xdat_cr2$Age)
colnames(xdat_cr2$Age) <- "Age"
...
# p-values collection
reg1_CSHRLD_pvalues <- data.frame(sapply(1:nrow(y_cor_control2), 
                                         function(i) {
                                           a1 <- y_cor_control2$Row[i]
                                           b1 <- y_cor_control2$Column[i]
                                           Y <- y_1[a1, b1, cn2shrld]
                                           fit<- lm(Y ~ xdat_cr2$Sex + xdat_cr2$Risk + xdat_cr2$Age)
                                           pvalues <- matrix(summary(fit)$coefficients[,4], nrow =1)
                                           colnames(pvalues) <- c("Intercept", "Sex", "Risk", "Age")
                                           return(pvalues)
...
#reg1_CSHR_pvalues_adj <- apply(reg1_CLR_pvalues, 1, p.adjust, method= "fdr")
# coefficient estimate collection
reg1_CSHRLD_coeffs <- data.frame(sapply(1:nrow(y_cor_control2), 
                                        function(i) {
                                          a1 <- y_cor_control2$Row[i]
                                          b1 <- y_cor_control2$Column[i]
                                          Y <- y_1[a1, b1, cn2shrld]
...
reg1_CSHRLD_pvalues_adjcoeff <- apply(reg1_CSHRLD_pvalues, 1, p.adjust, method= "fdr")

reg1_CSHRLD_pvalues_adj <- which(reg1_CSHRLD_pvalues_adjcoeff[,2] < 0.05)
reg1_CSHRLD_sex <- reg1_CSHRLD_coeffs[2, reg1_CSHRLD_pvalues_adj]

reg1_CSHRLD_pvalues_adj <- which(reg1_CSHRLD_pvalues_adjcoeff[,3] < 0.05)
reg1_CSHRLD_risk <- reg1_CSHRLD_coeffs[3, reg1_CSHRLD_pvalues_adj]

reg1_CSHRLD_pvalues_adj <- which(reg1_CSHRLD_pvalues_adjcoeff[,4] < 0.05)
reg1_CSHRLD_age <- reg1_CSHRLD_coeffs[4, reg1_CSHRLD_pvalues_adj]


# for Time = 2
# p-values collection
reg2_CSHRLD_pvalues <- data.frame(sapply(1:nrow(y_cor_control2), 
                                         function(i) {
                                           a1 <- y_cor_control2$Row[i]
                                           b1 <- y_cor_control2$Column[i]
                                           Y <- y_2[a1, b1, cn2shrld]
                                           fit<- lm(Y ~ xdat_cr2$Sex + xdat_cr2$Risk + xdat_cr2$Age)
                                           pvalues<- matrix(summary(fit)$coefficients[,4], nrow =1)
                                           colnames(pvalues) <- c("Intercept", "Sex", "Risk","Age")
                                           return(pvalues)
...
# coefficient estimate collection
reg2_CSHRLD_coeffs <- data.frame(sapply(1:nrow(y_cor_control2), 
                                        function(i) {
                                          a1 <- y_cor_control2$Row[i]
                                          b1 <- y_cor_control2$Column[i]
                                          Y <- y_2[a1, b1, cn2shrld]
...
reg2_CSHRLD_pvalues_adjcoeff <- apply(reg2_CSHRLD_pvalues, 1, p.adjust, method= "fdr")

reg2_CSHRLD_pvalues_adj <- which(reg2_CSHRLD_pvalues_adjcoeff[,2] < 0.05)
reg2_CSHRLD_sex <- reg2_CSHRLD_coeffs[2,reg2_CSHRLD_pvalues_adj]

reg2_CSHRLD_pvalues_adj <- which(reg2_CSHRLD_pvalues_adjcoeff[,3] < 0.05)
reg2_CSHRLD_risk <- reg2_CSHRLD_coeffs[3, reg2_CSHRLD_pvalues_adj]

reg2_CSHRLD_pvalues_adj <- which(reg2_CSHRLD_pvalues_adjcoeff[,4] < 0.05)
reg2_CSHRLD_age<- reg2_CSHRLD_coeffs[4,reg2_CSHRLD_pvalues_adj]

# for connectivity differences: Time = 2 - Time = 1
# p-values collection
reg21_CSHRLD_pvalues <- data.frame(sapply(1:nrow(y_cor_control2), 
                                          function(i) {
                                            a1 <- y_cor_control2$Row[i]
                                            b1 <- y_cor_control2$Column[i]
                                            Y <- y_2[a1, b1, cn2shrld] - y_1[a1, b1, cn2shrld]
                                            fit<- lm(Y ~ xdat_cr2$Sex + xdat_cr2$Risk + xdat_cr2$Age)
                                            pv<- matrix(summary(fit)$coefficients[,4], nrow =1)
                                            colnames(pv) <- c("Intercept", "Sex", "Risk","Age")
                                            return(pv)
...
# coeffcient estimate collection
reg21_CSHRLD_coeffs <- data.frame(sapply(1:nrow(y_cor_control2), 
                                         function(i) {
                                           a1 <- y_cor_control2$Row[i]
                                           b1 <- y_cor_control2$Column[i]
                                           Y <- y_2[a1, b1, cn2shrld] - y_1[a1, b1, cn2shrld]
...
reg21_CSHRLD_pvalues_adjcoeff <- apply(reg21_CSHRLD_pvalues, 1, p.adjust, method= "fdr")

reg21_CSHRLD_pvalues_adj <- which(reg21_CSHRLD_pvalues_adjcoeff[,3] < 0.05)
reg21_CSHRLD_risk<- reg21_CSHRLD_coeffs[3, reg21_CSHRLD_pvalues_adj]

reg21_CSHRLD_pvalues_adj<- which(reg21_CSHRLD_pvalues_adjcoeff[,4] < 0.05)
reg21_CSHRLD_age<- reg21_CSHRLD_coeffs[4,reg21_CSHRLD_pvalues_adj]


#############################
## For Control vs SHR(HD) ##
#############################

cn2shrhd <- c(cn, shrhd26)

# subset the covariate data for control and low risk groups
xdat_cr2 <- Xdat[cn2shrhd, ]
#xdat_cr2$Risk <- relevel(xdat_cr2$Risk, ref = "")
xdat_cr2$Age <- scale(xdat_cr2$Age)
colnames(xdat_cr2$Age) <- "Age"
...
# p-values collection
reg1_CSHRHD_pvalues <- data.frame(sapply(1:nrow(y_cor_control2), 
                                         function(i) {
                                           a1 <- y_cor_control2$Row[i]
                                           b1 <- y_cor_control2$Column[i]
                                           Y <- y_1[a1, b1, cn2shrhd]
                                           fit<- lm(Y ~ xdat_cr2$Sex + xdat_cr2$Risk + xdat_cr2$Age)
                                           pvalues <- matrix(summary(fit)$coefficients[,4], nrow =1)
                                           colnames(pvalues) <- c("Intercept", "Sex", "Risk", "Age")
                                           return(pvalues)
...
#reg1_CSHR_pvalues_adj <- apply(reg1_CLR_pvalues, 1, p.adjust, method= "fdr")
# coefficient estimate collection
reg1_CSHRHD_coeffs <- data.frame(sapply(1:nrow(y_cor_control2), 
                                        function(i) {
                                          a1 <- y_cor_control2$Row[i]
                                          b1 <- y_cor_control2$Column[i]
                                          Y <- y_1[a1, b1, cn2shrhd]
...
reg1_CSHRHD_pvalues_adjcoeff <- apply(reg1_CSHRHD_pvalues, 1, p.adjust, method= "fdr")

reg1_CSHRHD_pvalues_adj <- which(reg1_CSHRHD_pvalues_adjcoeff[,2] < 0.05)
reg1_CSHRHD_sex <- reg1_CSHRHD_coeffs[2, reg1_CSHRHD_pvalues_adj]

reg1_CSHRHD_pvalues_adj <- which(reg1_CSHRHD_pvalues_adjcoeff[,3] < 0.05)
reg1_CSHRHD_risk <- reg1_CSHRHD_coeffs[3, reg1_CSHRHD_pvalues_adj]

reg1_CSHRHD_pvalues_adj <- which(reg1_CSHRHD_pvalues_adjcoeff[,4] < 0.05)
reg1_CSHRHD_age <- reg1_CSHRLD_coeffs[4, reg1_CSHRHD_pvalues_adj]


# for Time = 2
# p-values collection
reg2_CSHRHD_pvalues <- data.frame(sapply(1:nrow(y_cor_control2), 
                                         function(i) {
                                           a1 <- y_cor_control2$Row[i]
                                           b1 <- y_cor_control2$Column[i]
                                           Y <- y_2[a1, b1, cn2shrhd]
                                           fit<- lm(Y ~ xdat_cr2$Sex + xdat_cr2$Risk + xdat_cr2$Age)
                                           pvalues<- matrix(summary(fit)$coefficients[,4], nrow =1)
                                           colnames(pvalues) <- c("Intercept", "Sex", "Risk","Age")
                                           return(pvalues)
...
# coefficient estimate collection
reg2_CSHRHD_coeffs <- data.frame(sapply(1:nrow(y_cor_control2), 
                                        function(i) {
                                          a1 <- y_cor_control2$Row[i]
                                          b1 <- y_cor_control2$Column[i]
                                          Y <- y_2[a1, b1, cn2shrhd]
...
reg2_CSHRHD_pvalues_adjcoeff <- apply(reg2_CSHRHD_pvalues, 1, p.adjust, method= "fdr")

reg2_CSHRHD_pvalues_adj <- which(reg2_CSHRHD_pvalues_adjcoeff[,2] < 0.05)
reg2_CSHRHD_sex<- reg2_CSHRHD_coeffs[2,reg2_CSHRHD_pvalues_adj]

reg2_CSHRHD_pvalues_adj <- which(reg2_CSHRHD_pvalues_adjcoeff[,3] < 0.05)
reg2_CSHRHD_risk <- reg2_CSHRHD_coeffs[3, reg2_CSHRHD_pvalues_adj]

reg2_CSHRHD_pvalues_adj <- which(reg2_CSHRHD_pvalues_adjcoeff[,4] < 0.05)
reg2_CSHRHD_age<- reg2_CSHRHD_coeffs[4,reg2_CSHRHD_pvalues_adj]

# for connectivity differences: Time = 2 - Time = 1
# p-values collection
reg21_CSHRHD_pvalues <- data.frame(sapply(1:nrow(y_cor_control2), 
                                          function(i) {
                                            a1 <- y_cor_control2$Row[i]
                                            b1 <- y_cor_control2$Column[i]
                                            Y <- y_2[a1, b1, cn2shrhd] - y_1[a1, b1, cn2shrhd]
                                            fit<- lm(Y ~ xdat_cr2$Sex + xdat_cr2$Risk + xdat_cr2$Age)
                                            pv<- matrix(summary(fit)$coefficients[,4], nrow =1)
                                            colnames(pv) <- c("Intercept", "Sex", "Risk","Age")
                                            return(pv)
...
# coeffcient estimate collection
reg21_CSHRHD_coeffs <- data.frame(sapply(1:nrow(y_cor_control2), 
                                         function(i) {
                                           a1 <- y_cor_control2$Row[i]
                                           b1 <- y_cor_control2$Column[i]
                                           Y <- y_2[a1, b1, cn2shrhd] - y_1[a1, b1, cn2shrhd]
...
reg21_CSHRHD_pvalues_adjcoeff <- apply(reg21_CSHRHD_pvalues, 1, p.adjust, method= "fdr")

reg21_CSHRHD_pvalues_adj <- which(reg21_CSHRHD_pvalues_adjcoeff[,3] < 0.05)
reg21_CSHRHD_risk<- reg21_CSHRHD_coeffs[3, reg21_CSHRHD_pvalues_adj]

reg21_CSHRHD_pvalues_adj<- which(reg21_CSHRHD_pvalues_adjcoeff[,4] < 0.05)
reg21_CSHRHD_age<- reg21_CSHRHD_coeffs[4,reg21_CSHRHD_pvalues_adj]


# histograms of age coefficients for comparison of Control vs SHR
age_CSHRLD <- data.frame(Coefficient= c(as.numeric(reg1_CSHRLD_age), 
                                        as.numeric(reg2_CSHRLD_age), 
                                        as.numeric(reg21_CSHRLD_age)),
                         Design = c(rep("Time 1, C vs SHR(LD)", length(reg1_CSHRLD_age)),
                                    rep("Time 2, C vs SHR(LD)", length(reg2_CSHRLD_age)),
                                    rep("Difference, C vs SHR(LD)", 
                                        length(reg21_CSHRLD_age))))

age_CSHRHD <- data.frame(Coefficient= c(as.numeric(reg1_CSHRHD_age), 
                                        as.numeric(reg2_CSHRHD_age), 
                                        as.numeric(reg21_CSHRHD_age)),
                         Design = c(rep("Time 1, C vs SHR(HD)", length(reg1_CSHRHD_age)),
                                    rep("Time 2, C vs SHR(HD)", length(reg2_CSHRHD_age)),
                                    rep("Difference, C vs SHR(HD)", 
                                        length(reg21_CSHRHD_age))))

age_plotdat<- rbind.data.frame(age_CSHRLD, age_CSHRHD)

ggplot(age_plotdat, aes(Coefficient)) + geom_histogram() + 
  facet_wrap(~ factor(Design, c("Time 1, C vs SHR(LD)", "Time 2, C vs SHR(LD)", "Difference, C vs SHR(LD)",
                                "Time 1, C vs SHR(HD)", "Time 2, C vs SHR(HD)", "Difference, C vs SHR(HD)")) )+
  xlab("Coefficient of Age")+
  ylab("Count of significant coefficients after FDR correction")

ggsave(file="Rplot_coeff_massiveregression_ageCSHR.png", height = 6, width =8.5,
       units ="in", dpi= 300)

# histograms of risk coefficients for comparison of Control vs SHR
risk_CSHRLD <- data.frame(Coefficient= c(as.numeric(reg1_CSHRLD_risk), 
                                         as.numeric(reg2_CSHRLD_risk), 
                                         as.numeric(reg21_CSHRLD_risk)),
                          Design = c(rep("Time 1, C vs SHR(LD)", length(reg1_CSHRLD_risk)),
                                     rep("Time 2, C vs SHR(LD)", length(reg2_CSHRLD_risk)),
                                     rep("Difference, C vs SHR(LD)", 
                                         length(reg21_CSHRLD_risk))))

risk_CSHRHD <- data.frame(Coefficient= c(as.numeric(reg1_CSHRHD_risk), 
                                         as.numeric(reg2_CSHRHD_risk), 
                                         as.numeric(reg21_CSHRHD_risk)),
                          Design = c(rep("Time 1, C vs SHR(HD)", length(reg1_CSHRHD_risk)),
                                     rep("Time 2, C vs SHR(HD)", length(reg2_CSHRHD_risk)),
                                     rep("Difference, C vs SHR(HD)", 
                                         length(reg21_CSHRHD_risk))))

risk_plotdat <- rbind.data.frame(risk_CSHRLD, risk_CSHRHD)

ggplot(risk_plotdat, aes(Coefficient)) + geom_histogram() + 
  facet_wrap(~ factor(Design, c("Time 1, C vs SHR(LD)", "Time 2, C vs SHR(LD)", 
                                "Difference, C vs SHR(LD)",
                                "Time 1, C vs SHR(HD)", "Time 2, C vs SHR(HD)", 
                                "Difference, C vs SHR(HD)")) )+
  xlab("Coefficient of Risk")+
  ylab("Count of significant coefficients after FDR correction") 
...
# histograms of sex coefficients for comparison of Control vs Standard-High Risk
sex_CSHRLD <- data.frame(Coefficient= c(as.numeric(reg1_CSHRLD_sex), 
                                        as.numeric(reg2_CSHRLD_sex)),
                         Design = c(rep("Time 1, C vs SHR(LD)", length(reg1_CSHRLD_sex)),
                                    rep("Time 2, C vs SHR(LD)", length(reg2_CSHRLD_sex))))

sex_CSHRHD <- data.frame(Coefficient= c(as.numeric(reg1_CSHRHD_sex), 
                                        as.numeric(reg2_CSHRHD_sex)),
                         Design = c(rep("Time 1, C vs SHR(HD)", length(reg1_CSHRHD_sex)),
                                    rep("Time 2, C vs SHR(HD)", length(reg2_CSHRHD_sex))))

sex_plotdat <- rbind.data.frame(sex_CSHRLD, sex_CSHRHD)
...
##########################################################################################
# We gather the data for every shortlisted brain edge for every patient in our study and 
# check that the edge is extreme or not, if extreme, we also note the position of the edge i.e.
# lower or upper tail of the control distribution. We perform this exercise at time =1, 2, and at the 
# difference of connectivity. 


# first consider the connectivity differences
# get the p-value for every connectivity differences; rows represent the patients, columns represent the edges.
...
roi_diff21 <- cbind.data.frame(Edges= seq(1: nrow(y_cor_control2)), 
                               y_cor_control2[, -1]) 
...
m_t1 <- matrix(NA, nrow= lp, ncol= nrow(y_cor_control2))  # at the start
m_t2 <- matrix(NA, nrow= lp, ncol= nrow(y_cor_control2))  # at the 120 weeks study
m_t1_tail <- matrix(NA, nrow= lp, ncol= nrow(y_cor_control2))  # at the start
m_t2_tail <- matrix(NA, nrow= lp, ncol= nrow(y_cor_control2))  # at the 120 weeks study
...
RE_plotrisk <- data.frame( Extreme= c(roiedge_plot1$ExtremePatients, 
                                      roiedge_plot2$ExtremePatients,
                                      roiedge_plot21$ExtremePatients), 
                           Time= rep(c("RE-1", "W-120", "Difference"), 
                                     each= nrow(roiedge_plot1)) )

RE_plotrisk$Time <- factor(RE_plotrisk$Time, levels= c("RE-1", "W-120","Difference"))
ggplot(RE_plotrisk, aes(y= Extreme, fill = Time)) +
  geom_boxplot()+
  guides(x = guide_axis(title = NULL, position = "NULL"))

RE_plotrisk2 <- data.frame( Extreme= c(roiedge_plot1$LR, roiedge_plot1$SHR,  
                                       roiedge_plot2$LR, roiedge_plot2$SHR,
                                       roiedge_plot21$LR, roiedge_plot21$SHR), 
                            Time= rep(c("RE-1", "W-120", "Difference"), 
                                      each= 2*nrow(roiedge_plot1)),
                            Risk= rep(rep(c("LR","SHR"), each= length(roiedge_plot1$LR)),
                                      3) )

RE_plotrisk2$Time <- factor(RE_plotrisk2$Time, levels= c("RE-1", "W-120","Difference"))
#RE_plotrisk2$Risk <- factor(RE_plotrisk2$Time, levels= c("LR", "SHR"))
ggplot(RE_plotrisk2, aes(y= Extreme, x=Risk, fill = Time)) +
  geom_boxplot()

RE_plotrisk3 <- data.frame( Extreme= c(roiedge_plot1$LR_LT, roiedge_plot1$LR_UT,
                                       roiedge_plot1$SHR_LT, roiedge_plot1$SHR_UT,  
                                       roiedge_plot2$LR_LT, roiedge_plot2$LR_UT,
                                       roiedge_plot2$SHR_LT, roiedge_plot2$SHR_UT,
                                       roiedge_plot21$LR_LT, roiedge_plot21$LR_UT,
                                       roiedge_plot21$SHR_LT, roiedge_plot21$SHR_UT),
                            Time= rep(c("RE-1", "W-120", "Difference"), 
                                      each= 4*nrow(roiedge_plot1)),
                            Risk= rep(rep(c("LR(l-tail)", "LR(u-tail)", 
                                            "SHR(l-tail)", "SHR(u-tail)"),
                                          each = length(roiedge_plot1$LR)),3) )

RE_plotrisk3$Time <- factor(RE_plotrisk3$Time, levels= c("RE-1", "W-120","Difference"))
ggplot(RE_plotrisk3, aes(y= Extreme, fill = Time)) +
  geom_boxplot()+
  facet_wrap(~ Risk)+
  theme_bw()+
  labs(y ="Extreme edges count")+
  guides(x = guide_axis(title = NULL, position = "NULL"))

ggsave(file="Rplot_extremeedges_boxplots.png", height = 6, width = 6.5,
       units ="in", dpi= 300)
...
#t.test(`IQ difference` ~ as.character(Risk), data = plotdat1, var.equal=T)
#t.test(`IQ difference` ~ as.character(Sex), data = plotdat2, var.equal=T)

t1 <- subset(plotdat2, `Risk & ITT` == "LR(LD)")[,1]
t2 <- subset(plotdat2, `Risk & ITT` == "LR(HD)")[,1]
t3 <- subset(plotdat2, `Risk & ITT` == "SHR(LD)")[,1]
t4 <- subset(plotdat2, `Risk & ITT` == "SHR(HD)")[,1]
...
for (j in 1:nrow(roi_diff21)) {
  ...
  if(Fpv< 0.05 & aoft$`Pr(>F)`[2] <0.05) {
    ...
    roi_diff_reg <- rbind.data.frame(roi_diff_reg, data.frame("Edge"= roi_diff21$Edges[j],
                                                              ...
                                                              "LR(HD)"= ft_summ$coefficients[3,1],
                                                              "SHR(LD)"= ft_summ$coefficients[4,1],
                                                              "SHR(HD)"= ft_summ$coefficients[5,1],
                                                              "Yd:LR(HD)"= ft_summ$coefficients[6,1],
                                                              "Yd:SHR(LD)"= ft_summ$coefficients[7,1],
                                                              "Yd:SHR(HD)"=ft_summ$coefficients[8,1],
...
p1 <- ggplot(dat10, aes(x=`Connectivity difference`, y=`IQ difference`, 
                        color = Groups)) +
  geom_point()+
...
# working memory tests include wechsler, auditory WM
...
##########################################################################################
# Next we gather the edges that contribute significantly to the Wechsler score prediction
##########################################################################################

# for the risk group and ITT groups LR(LD), LR(HD), SHR(LD), SHR(HD)
...
for (j in 1:nrow(roi_diff21)) {
  ...
  dat10 <- cbind.data.frame(temp34[,-2], "Connectivity difference"= yd, 
...
write.csv(roi_diff_reg, "ROI_regression_Wechsler1.csv")
...
dat10 <- cbind.data.frame("Connectivity difference"= yd_vec, 
                          Wechsler = rep(as.numeric(temp34$wech.totalds.scs), 6),
                          "Edge"= rep(c("R PFcm vs R OP2-3/VS", "R Dorsal 24d vs R 5m Ventral",
                                        "L PHT vs L TE1 posterior", "L TPOccipital Junction 1 vs 2",
                                        "L 7m vs L 7p", "L 31p vs L 7m"), each= nrow(temp34)),
                          "Groups" = rep(temp34$Group.ITT, each= 6)) 
...
p1 <- ggplot(dat10, aes(x=`Connectivity difference`, y= Wechsler, 
                        color = Groups)) +
  geom_point() +
...
temp35<- subset(temp34, Group.ITT %in% c("SHR(LD)", "SHR(HD)"))

# for the risk group and ITT groups SHR(LD), SHR(HD) for another analysis ~
...
for (j in 1:nrow(roi_diff21)) {
  ...
  y1d <- y_diff[roi_diff21$Row[j], roi_diff21$Column[j], ]
  yd <- subset(y1d, names(y1d) %in% temp35$MRN)
  
  dat10 <- cbind.data.frame(temp35[,-2], "Connectivity difference"= yd, 
                            "Wechsler"= as.numeric(temp35$wech.totalds.scs))
  ...
  Fpv <- pf(ft_summ$fstatistic[1], ft_summ$fstatistic[2], ft_summ$fstatistic[3], 
            lower.tail = F)
  
  ats<- t.test(`Connectivity difference` ~ Group.ITT, data= dat10)
  # 
  if(Fpv< 0.05 & aoft$`Pr(>F)`[2] <0.05) {
    print(j)
    roi_diff_reg <- rbind.data.frame(roi_diff_reg, data.frame("Edge"= roi_diff21$Edges[j],
                                                              ...
                                                              "Intercept"= ft_summ$coefficients[1,1],
                                                              "Connectivity"= ft_summ$coefficients[2,1],
                                                              "SHR(HD)"= ft_summ$coefficients[3,1],
                                                              "Yd:SHR(HD)"= ft_summ$coefficients[4,1],
                                                              "Adj.R2"= ft_summ$adj.r.squared, 
                                                              "Fpvalue"=Fpv,
                                                              "pvalsConnectivityDiff"= ats$p.value))
...
write.csv(roi_diff_reg, "ROI_regression_Wechsler2.csv")
...
dat10 <- cbind.data.frame("Connectivity difference"= yd_vec, 
                          Wechsler = rep(as.numeric(temp35$wech.totalds.scs), 
                                         nrow(roi_diff_reg2)),
                          "Edge"= as.factor(paste(rep(as.factor(c(1:11)), each= nrow(temp35)),
                                                  rep(roi_diff_reg2$pvalsConnectivityDiff, 
                                                      each= nrow(temp35)), sep = ",")),
                          "Groups"= rep(temp35$Group.ITT, each= nrow(roi_diff_reg2))) 
...
t1<- subset(plotdat2, `Risk & ITT` == "SHR(LD)")[,1]
t2<- subset(plotdat2, `Risk & ITT` == "SHR(HD)")[,1]
t3<- subset(plotdat2, `Risk & ITT` == "LR(HD)")[,1]
...
pv<- sapply(1:nrow(y_cor_control2), function(i) {
  t1<- y_diff[y_cor_control2$Row[i], y_cor_control2$Column[i], shrld26]
  t2<- y_diff[y_cor_control2$Row[i], y_cor_control2$Column[i], shrhd26]
  a<- t.test(t1, t2, var.equal =TRUE)
  a$p.value
})

hist(p.adjust(pv, method = "fdr"))
hist(pv)
abline(v= 0.05, col= "red")

corr_dat <- NA
for (i in 1:nrow(y_cor_control2)) {
  corrm_dat <- cbind.data.frame(corr_dat, 
                                y_diff[y_cor_control2$Row[i], y_cor_control2$Column[i], ]) 
  
}

corrm_dat <- corrm_dat[, -1]
colnames(corrm_dat) <- as.character(c(1:671))
a <- princomp( cor(corrm_dat))
...
mrn1_riskitt$Group.ITT <- sapply(1:nrow(mrn1_riskitt), function(i) {
  ...
  if(mrn1_riskitt$Risk[i] == "LR" & mrn1_riskitt$IT.total[i] <= 26 ) {"LR(LD)" }
  else if (mrn1_riskitt$Risk[i] == "SHR" & mrn1_riskitt$IT.total[i] <= 26) {"SHR(LD)"}
  else if (mrn1_riskitt$Risk[i] == "SHR" & mrn1_riskitt$IT.total[i] >= 27) {"SHR(HD)"}
})
...
t1<- subset(mrn1_riskitt, Group.ITT == "SHR(LD)")$Scores
t2<- subset(mrn1_riskitt, Group.ITT == "LR(LD)")$Scores
t3<- subset(mrn1_riskitt, Group.ITT == "SHR(HD)")$Scores
...
hdist2_C_LRLD19 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11 <- y_2[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11 <- l11[cn]
  y11 <- l11[lrld19]
  statip::hellinger(x11, y11)
...
hdist1_C_LRLD19 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_1[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[lrld19]
  statip::hellinger(x11, y11)
...
hdat_C_LRLD19 <- data.frame(HDist= c(hdist1_C_LRLD19, 
                                     hdist2_C_LRLD19),
                            Time= c(rep("1", nrow(y_cor_control2)), 
                                    rep("2", nrow(y_cor_control2))) )

ggplot(hdat_C_LRLD19, aes(x= HDist)) +
  geom_histogram(aes(fill = Time, color= Time, alpha =0.5), color= "black",
                 position = "identity") +
...
hdist2_C_LRHD19 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_2[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[lrhd19]
  statip::hellinger(x11, y11, lower = 0, upper = Inf)
...
hdist1_C_LRHD19 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_1[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[lrhd19]
  hdist(x11, y11, lower = 0, upper = Inf)
...
hdat_C_LRHD19 <- data.frame(HDist= c(hdist1_C_LRHD19, 
                                     hdist2_C_LRHD19),
                            Time= c(rep("1", nrow(y_cor_control2)), 
                                    rep("2", nrow(y_cor_control2))) )

# Hellinger distance between groups LR with ITT < 20 & LR with ITT >=20
hdist2_LRLD19_LRHD19 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_2[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[lrld19]
  y11<- l11[lrhd19]
  statip::hellinger(x11, y11, lower = 0, upper = Inf)
...
hdist1_LRLD19_LRHD19 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_1[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[lrld19]
  y11<- l11[lrhd19]
  statip::hellinger(x11, y11, lower = 0, upper = Inf)
...
hdat_LRLD19_LRHD19 <- data.frame(HDist= c(hdist1_LRLD19_LRHD19, 
                                          hdist2_LRLD19_LRHD19),
                                 Time= c(rep("1", nrow(y_cor_control2)), 
                                         rep("2", nrow(y_cor_control2))) )


boxplot(HDist ~ Time, data = hdat_C_LRHD19, ylab="2-Wasserstein Distance",
        main= "Distance: C vs LR at HD", pch=20, cex=.5)


# Hellinger distance between Control and SHR with Low Dose >= 21
hdist2_C_SHRLD26 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_2[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[shrld26]
  statip::hellinger(x11, y11, lower = 0, upper = Inf)
...
hdist1_C_SHRLD26 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_1[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[shrld26]
  statip::hellinger(x11, y11, lower = 0, upper = Inf)
...
hdat_C_SHRLD26 <- data.frame(HDist= c(hdist1_C_SHRLD26, 
                                      hdist2_C_SHRLD26),
                             Time= c(rep("1", nrow(y_cor_control2)), 
                                     rep("2", nrow(y_cor_control2))) )

ggplot(hdat_C_SHRLD26, aes(x= HDist)) +
  geom_histogram(aes(fill = Time, color= Time, alpha =0.5), color= "black",
                 position = "identity") +
  labs(x="Hellinger Distance",
       title = "Distance between C & SHR at LD")

boxplot(HDist ~ Time, data = hdat_C_SHRLD26, ylab="Hellinger Distance",
        main= "Distance: C vs LR at HD", pch=20, cex=.5)

# Hellinger distance between SHR with High Dose >= 27 & Control groups
# at time = 2
hdist2_C_SHRHD26 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_2[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[shrhd26]
  statip::hellinger(x11, y11, lower =0, upper = Inf)
...
hdist1_C_SHRHD26 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_1[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[cn]
  y11<- l11[shrhd26]
  hdist(x11, y11, lower =0, upper = Inf)
...
hdat_C_SHRHD26<- data.frame(HDist = c(hdist1_C_SHRHD26, 
                                      hdist2_C_SHRHD26),
                            Time= c(rep("1", nrow(y_cor_control2)), 
                                    rep("2", nrow(y_cor_control2))) )

ggplot(hdat_C_SHRHD26, aes(x= HDist)) +
  geom_histogram(aes(fill = Time, color= Time, alpha =0.5), color= "black",
                 position = "identity") +
  labs(y="Hellinger Distance",
       title = "Distance between C & SHR at HD")

ggplot(hdat_C_SHRHD26, aes(x= Time, y= HDist, fill = Time)) +
  geom_boxplot()+
  labs(y ="Hellinger Distance", title = "Distance: C vs SHR at HD")

# Hellinger distance between SHR LD group & SHR HD group
# at time = 2
hdist2_SHRLD_SHRHD26 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11 <- y_2[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11 <- l11[shrld26]
  y11 <- l11[shrhd26]
  statip::hellinger(x11, y11, lower =0, upper = Inf)
...
# at time = 1 (hdist function was used because round-off error was reported while 
# using the statip::hellinger function)
hdist1_SHRLD_SHRHD26 <- sapply(1:nrow(y_cor_control2), function(i) {
  l11<- y_1[y_cor_control2$Row[i], y_cor_control2$Column[i], ]
  x11<- l11[shrld26]
  y11<- l11[shrhd26]
  statip::hellinger(x11, y11, lower =0, upper = Inf)
...
hdat_SHRLD_SHRHD26 <- data.frame(HDist = c(hdist1_SHRLD_SHRHD26, 
                                           hdist2_SHRLD_SHRHD26),
                                 Time= c(rep("1", nrow(y_cor_control2)), 
                                         rep("2", nrow(y_cor_control2))) )

ggplot(hdat_SHRLD_SHRHD26, aes(x= HDist)) +
  geom_histogram(aes(fill = Time, color= Time, alpha =0.5), color= "black",
                 position = "identity") +
  labs(y="Hellinger Distance",
       title = "Distance between C & SHR(HD)")

ggplot(hdat_SHRLD_SHRHD26, aes(x= Time, y= HDist, fill = Time)) +
  geom_boxplot()+
...
t.test(hdist1_C_LRLD19, hdist2_C_LRLD19, paired =TRUE, alternative ="greater")

t.test(hdist1_LRLD19_LRHD19, hdist2_LRLD19_LRHD19, paired =TRUE, alternative ="greater")

t.test(hdist1_C_LRHD19, hdist2_C_LRHD19, paired =TRUE, alternative ="greater")

t.test(hdist1_C_SHRLD26, hdist2_C_SHRLD26, paired =TRUE, alternative ="greater")

t.test(hdist1_C_SHRHD26, hdist2_C_SHRHD26, paired =TRUE,
       alternative= "greater")

t.test(hdist1_SHRLD_SHRHD26, hdist2_SHRLD_SHRHD26, paired =TRUE,
       alternative= "greater")

hdat_all <- data.frame(rbind.data.frame(hdat_C_LR, hdat_C_SHR, hdat_LR_SHR,
                                        hdat_C_SHRLD26, hdat_C_SHRHD26,
                                        hdat_SHRLD_SHRHD26), 
